...
if (conf.LANG == 'ML') voicechat_dsc = 'Alexa ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് ഉപയോഗിച്ച് വോയ്‌സ് ചാറ്റ് ആരംഭിക്കുക.', reply_alexa = '*ഏത് വോയ്‌സ് സന്ദേശത്തിനും മറുപടി നൽകുക!*'
...
if (conf.LANG == 'EN') {
    ...
    succ_on = 'Alexa artificial intelligence Successfully Turned on! Please wait a bit! ✅'
    succ_off = 'Alexa artificial intelligence Successfully Turned off! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fullalexa_dsc = 'നിങ്ങളുടെ അക്കൗണ്ട് ഒരു പൂർണ്ണ ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Alexa ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് ഇതിനകം തന്നെ പ്രവർത്തിക്കുന്നു.'
    already_off = 'Alexa ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് നിലവിൽ ഓഫ് ആണ്.'
    succ_on = 'Alexa ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് വിജയകരമായി ഓൺ ചെയ്തു! ദയവായി അൽപ്പ നേരം കാത്തിരിക്കൂ! ✅'
    succ_off = 'ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് വിജയകരമായി ഓഫാക്കി! ദയവായി അൽപ്പ നേരം കാത്തിരിക്കൂ! ☑️'
}
if (conf.LANG == 'ID') {
    fullalexa_dsc = 'Ubah akun Anda menjadi chatbot ai sepenuhnya!'
    already_on = 'Kecerdasan buatan Alexa sudah aktif.'
    already_off = 'Kecerdasan buatan Alexa saat ini tidak aktif.'
    succ_on = 'Kecerdasan buatan Alexa Berhasil Diaktifkan! Harap tunggu sebentar! ✅️'
    succ_off = 'Kecerdasan buatan Alexa Berhasil Dimatikan! Harap tunggu sebentar! ☑️'
}

WhatsAlexa.addCommand({ pattern: 'fullalexa ?(.*)', desc: fullalexa_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
    if (match[1] == 'on') {
